...
export async function HowItWorks() {
  ...
  return (
    <section className="py-32 bg-surface relative z-40 px-4 sm:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          ...
          <div className="hidden md:block absolute top-20 start-[16.67%] end-[16.67%] h-px bg-gradient-to-r from-transparent via-primary-container/30 to-transparent rtl:bg-gradient-to-l" />

          {steps.map((step, i) => (
            <Reveal key={step.num} delay={i * 100}>
              <div className="group relative p-10 glass-panel rounded-[2rem] hover:bg-surface-container-low transition-all duration-500">
                <div className="text-6xl font-black text-primary/10 absolute top-4 end-10 group-hover:text-primary/20 transition-colors font-headline">
                  {step.num}
                </div>
                <div className="w-14 h-14 bg-gradient-to-br from-primary-container/20 to-primary/10 rounded-2xl flex items-center justify-center mb-8">
                  {step.icon}
                </div>
                <h3 className="text-2xl font-headline font-bold mb-4">{t(`${step.key}Title`)}</h3>
                <p className="text-on-surface/60 leading-relaxed">{t(`${step.key}Desc`)}</p>
              </div>
            </Reveal>
          ))}
        </div>
